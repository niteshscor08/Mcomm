...
//            return false;
    ...
    public void Connect(String cookie) {
        serverConnection.connect(cookie);
    }
    ...
    public void receivedNewMessage(JSONObject obj) {
        try {
            ...
            switch (type) {
                ...
                case timeout: {
//                    handle.
                    Timber.d("TIMEOUT TIMEOUT TIMEOUT !!! !!!");
                }
                case hangup: {
                    if(handle != null) {
                        handle.hangUp();
                ...
                case media: {
                    if (handle != null) {
                        handle.onMedia(obj.getString("type"), obj.getBoolean("receiving") == Boolean.TRUE);
                    }
                }
            }
        } catch (JSONException ex) {
    ...
    public void onError(Exception ex) {
        ex.printStackTrace();
        gatewayObserver.onCallbackError("Error connected to Janus gateway. Exception: " + ex);
    }
    //endregion
    ...
    public IJanusMessenger getServerConnection() {
